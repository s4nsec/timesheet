...
def analyze(
    ...
    last_day = None
    for row in [
        x for x in timesheet_log.entries if x[0] <= string_stop and x[1] >= string_start
    ]:
    ...
    if left:
        if week_hours is None:
            print("Week hours not configured")
            return
        time_left = max(timedelta(hours=week_hours) - dur, timedelta(0))
...
def main(argv):
    ...
    if timesheet_logfile is None or timesheet_statefile is None:
        print("Configuration error: timesheet or state file path is None")
        return
    ...
    if command == "start":
        ...
        if ret:
            ...
            yn = input("the timer is already going.  start a new one? [Y/n] ")
            if yn.lower() == "n":
        ...
        if argument != "":
            ...
            if (stoptime - starttime).total_seconds() < 0:
                print("stop time cannot be earlier than stop time")
                return
        ...
        if argument == "":
            ...
            analyze(
                timesheet_log, timesheet_state, period_start, period_stop, current=True
            )
        else:
            interpreted_date = util.interpretdate(argument)
            if interpreted_date is None:
                print("invalid date. try again.")
                return
            period_start = util.get_week_start(
                start_day, start_time, week=interpreted_date
            )
        ...
        analyze(
            timesheet_log, timesheet_state, period_start, period_stop, current=False
        )
        ...
        if argument == "":
            ...
            if interpreted_date is None:
                print("invalid date. try again.")
                return
            period_start = util.get_day_start(day=interpreted_date)
